...
@app.route("/new_contacts", methods=('GET', 'POST'))
def new_contacts():
    '''
    Create new contact
    '''
    ...
    return render_template('web/new_contacts.html', form=form)


@app.route("/edit_contacts/<id>", methods=('GET', 'POST'))
...
def contacts():
    ...
    Show alls contacts
    '''
    contacts = Contact.query.order_by(Contact.auditory).all()
...
def search():
    ...
    Search
    '''
    auditory_search = request.args.get('auditory')
    all_contacts = Contact.query.filter(
        Contact.auditory.contains(auditory_search)
        ).order_by(Contact.auditory).all()
